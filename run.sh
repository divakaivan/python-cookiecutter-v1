...
#    GITHUB_USERNAME - name of my github user, e.g. divakaivan
...
EOF
    ...
    git commit -m 'feat!: populated from `python-cookiecutter-v1` template'

    # if GH_TOKEN is set, set the remote url to it
    if [[ -n "$GH_TOKEN" ]]; then
        git remote set-url origin "https://$GITHUB_USERNAME:$GH_TOKEN@github.com/divakaivan/$REPO_NAME"
    fi
